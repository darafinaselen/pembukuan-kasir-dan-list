...
test.describe("Armada (Vehicles) Feature", () => {
  test("should display armada page", async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("Tambah Armada"), button:has-text("Tambah")'
    );
  ...
  test("should open add armada dialog", async ({ page }) => {
    ...
    const addButton = page
      .locator('button:has-text("Tambah Armada"), button:has-text("Tambah")')
      .first();
    ...
    // Check form fields
    await expect(
      page.locator('input[name="license_plate"], input[placeholder*="Plat"]')
    ).toBeVisible();
    await expect(
      page.locator('input[name="brand"], input[placeholder*="Merek"]')
    ).toBeVisible();
    await expect(
      page.locator('input[name="model"], input[placeholder*="Model"]')
    ).toBeVisible();
  });
  ...
  test("should create new armada", async ({ page }) => {
    ...
    const statusSelect = page.locator(
      'select[name="status"], [role="combobox"]:has-text("Status")'
    );
    ...
    const submitButton = page.locator(
      'button[type="submit"], button:has-text("Simpan"), button:has-text("Save")'
    );
  ...
  test("should filter armada by status", async ({ page }) => {
    ...
    const statusFilter = page.locator(
      'select[name="status"], button:has-text("Status"), [role="combobox"]:has-text("Status")'
    );
