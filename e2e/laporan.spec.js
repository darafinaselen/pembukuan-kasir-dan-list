...
test.describe('Laporan (Reports) Feature', () => {
  ...
  test('should display laporan page', async ({ page }) => {
    ...
    // Check for tabs
    await expect(page.locator('button:has-text("Laporan Transaksi")')).toBeVisible();
    await expect(page.locator('button:has-text("Laporan Laba Rugi")')).toBeVisible();
    await expect(page.locator('button:has-text("Laporan Pemasukan")')).toBeVisible();
    await expect(page.locator('button:has-text("Laporan Pengeluaran")')).toBeVisible();
    await expect(page.locator('button:has-text("Rekapitulasi")')).toBeVisible();
    await expect(page.locator('button:has-text("Laporan Kinerja")')).toBeVisible();
  });
  ...
  test('should display driver performance in laporan kinerja', async ({ page }) => {
    await page.goto('/laporan');
    await page.waitForLoadState('networkidle');
    ...
    if (await driverTab.isVisible()) {
      ...
      // Check table headers
      await expect(page.locator('text=/Nama Sopir|Total Trip|Total Jam Kerja/i')).toBeVisible({ timeout: 5000 });
    }
  });

  test('should display package performance in laporan kinerja', async ({ page }) => {
    await page.goto('/laporan');
    await page.waitForLoadState('networkidle');
    ...
    if (await packageTab.isVisible()) {
      ...
      // Check table headers
      await expect(page.locator('text=/Nama Paket|Frekuensi|Tipe/i')).toBeVisible({ timeout: 5000 });
    }
  });
