...
test.describe("Paket (Service Packages) Feature", () => {
  test("should display paket page", async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("Tambah Paket"), button:has-text("Tambah")'
    );
  ...
  test("should open add paket dialog", async ({ page }) => {
    ...
    const addButton = page
      .locator('button:has-text("Tambah Paket"), button:has-text("Tambah")')
      .first();
    ...
    // Check form fields
    await expect(
      page.locator('input[name="name"], input[placeholder*="Nama"]')
    ).toBeVisible();
  });
  ...
  test("should create new paket", async ({ page }) => {
    ...
    const typeSelect = page.locator(
      'select[name="type"], [role="combobox"]:has-text("Tipe")'
    );
  ...
  test("should view paket details", async ({ page }) => {
    ...
    const detailButtons = page.locator(
      'button:has-text("Detail"), button:has-text("View")'
    );
