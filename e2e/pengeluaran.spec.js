...
test.describe("Pengeluaran (Expenses) Feature", () => {
  test("should display pengeluaran page", async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("Tambah Pengeluaran"), button:has-text("Tambah")'
    );
  ...
  test("should open add pengeluaran dialog", async ({ page }) => {
    ...
    // Check form fields
    await expect(
      page.locator('input[name="date"], input[type="date"]')
    ).toBeVisible();
    await expect(
      page.locator(
        'select[name="category"], [role="combobox"]:has-text("Kategori")'
      )
    ).toBeVisible();
    await expect(
      page.locator('input[name="amount"], input[placeholder*="Jumlah"]')
    ).toBeVisible();
  });
  ...
  test("should filter pengeluaran by category", async ({ page }) => {
    ...
    const categoryFilter = page.locator(
      'select[name="category"], button:has-text("Kategori")'
    );
