...
test.describe("Transaksi (Transactions) Feature", () => {
  test("should display transaksi page", async ({ page }) => {
    ...
    const addButton = page.locator(
      'button:has-text("Tambah Transaksi"), button:has-text("Tambah")'
    );
    await expect(addButton.first()).toBeVisible();
  });

  test("should open transaction dialog when clicking add button", async ({
    page,
  }) => {
    await page.goto("/transaksi");
    await page.waitForLoadState("networkidle");
    ...
    const addButton = page
      .locator('button:has-text("Tambah Transaksi"), button:has-text("Tambah")')
      .first();
    ...
    // Check form fields
    await expect(
      page.locator('input[name="customer_name"], input[placeholder*="Nama"]')
    ).toBeVisible();
    await expect(
      page.locator(
        'input[name="customer_phone"], input[placeholder*="Telepon"]'
      )
    ).toBeVisible();
  });

  test("should check vehicle availability when dates change", async ({
    page,
  }) => {
    await page.goto("/transaksi");
    await page.waitForLoadState("networkidle");
    ...
    if (await checkoutInput.isVisible()) {
      ...
      const vehicleSelect = page.locator(
        'select[name="armadaId"], [role="combobox"]:has-text("Armada")'
      );
      ...
      const driverSelect = page.locator(
        'select[name="driverId"], [role="combobox"]:has-text("Sopir")'
      );
  ...
  test("should display transaction details", async ({ page }) => {
    ...
    const detailButtons = page.locator(
      'button:has-text("Detail"), button:has-text("View"), button[aria-label*="Detail"]'
    );
